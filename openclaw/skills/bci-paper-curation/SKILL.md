...
description: >
  Select and write weekly BCI/EEG paper curation outputs from bci-tracker candidate pools.
  Use when Codex or OpenClaw needs to read bci_candidates_{date}.json, choose representative
  brain-computer interface, neural interface, neural decoding, or clinical EEG papers, write
...
3. Select 4-6 papers when enough genuinely relevant candidates exist, with no more than 2 papers from any one source.
...
- Mark `not_enough=true` when fewer than 4 candidates are genuinely relevant and worth including. Do not pad the list with weak papers.
